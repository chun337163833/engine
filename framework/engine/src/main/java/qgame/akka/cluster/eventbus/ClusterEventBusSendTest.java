 ...
 * Created by kerr.
 */
public class ClusterEventBusSendTest {
    //ping ,pong
    ...
    public static class PingActor extends UntypedActor {
        ...
        public void onReceive(Object message) throws Exception {
            ...
            if (message instanceof SendTick) {
                ...
                eventBus.send(new ClusterEventBusMessage("pong", "pongTopic", new Ping(),ActorRef.noSender()))
                        .onReceive(new AbstractFunction2<Object, ActorRef, BoxedUnit>() {
                            @Override
                            public BoxedUnit apply(Object v1, ActorRef v2) {
                                System.out.println("@@@@@@@received message :" + v1 + " from :" + v2);
                                return BoxedUnit.UNIT;
                            }
                        })
                        .timeOut(10, new AbstractFunction0<BoxedUnit>() {
                            @Override
                        ...
                        .onException(new AbstractFunction1<Throwable, BoxedUnit>() {
                            @Override
                            public BoxedUnit apply(Throwable v1) {
                                System.out.println("@@@@@@@ message handle exception :" + v1);
                                return BoxedUnit.UNIT;
                            }
                        }).run();
                return;
